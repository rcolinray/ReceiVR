...
function Start () {
	...
	if(gun_type == GunType.AUTOMATIC){
		...
		if(Random.Range(0,2) == 0){
			round_in_chamber = Instantiate(casing_with_bullet, transform.Find("point_chambered_round").position, transform.Find("point_chambered_round").rotation);
	...
	if(gun_type == GunType.REVOLVER){
		...
		for(var i=0; i<cylinder_capacity; ++i){
			...
			cylinders[i].object = Instantiate(casing_with_bullet, extractor_rod.Find(name).position, extractor_rod.Find(name).rotation);
...
function ChamberRoundFromMag() : boolean {
	...
	if(magazine_instance_in_gun && MagScript().NumRounds() > 0 && mag_stage == MagStage.IN){
		if(!round_in_chamber){
			...
			round_in_chamber = Instantiate(casing_with_bullet, transform.Find("point_load_round").position, transform.Find("point_load_round").rotation);
	...
	if((pressure_on_trigger == PressureState.INITIAL || action_type == ActionType.DOUBLE) && !slide_lock && thumb_on_hammer == Thumb.OFF_HAMMER && hammer_cocked == 1.0 && safety_off == 1.0 && (auto_mod_stage == AutoModStage.ENABLED || !fired_once_this_pull)){
		...
		if(gun_type == GunType.AUTOMATIC && slide_amount == 0.0){
			...
			if(round && cylinders[which_chamber].can_fire){
				...
				Instantiate(muzzle_flash, transform.Find("point_muzzleflash").position, transform.Find("point_muzzleflash").rotation);
				bullet = Instantiate(bullet_obj, transform.Find("point_muzzle").position, transform.Find("point_muzzle").rotation);
				bullet.GetComponent(BulletScript).SetVelocity(transform.forward * 251.0);
				rotation_transfer_y += Random.Range(1.0,2.0);
				rotation_transfer_x += Random.Range(-1.0,1.0);
				recoil_transfer_x -= Random.Range(150.0,300.0);
		...
		if(yolk){
			...
			if(cylinder_assembly){
				...
				if(extractor_rod){
					...
					cylinders[best_chamber].object = Instantiate(casing_with_bullet, extractor_rod.Find(name).position, extractor_rod.Find(name).rotation);
