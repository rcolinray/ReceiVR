...
function WasShot(obj : GameObject, pos : Vector3, vel : Vector3) {
	if(transform.parent && transform.parent.gameObject.name == "gun pivot"){
		var x_axis = transform.Find("point_pivot").rotation * Vector3(1,0,0);
		var y_axis = transform.Find("point_pivot").rotation * Vector3(0,1,0);
		var z_axis = transform.Find("point_pivot").rotation * Vector3(0,0,1);
...
function UpdateStationaryTurret() {
	...
	gun_pivot.RotateAround(
		...
		transform.Find("point_pivot").rotation * Vector3(1,0,0),
		rotation_x.state);
	gun_pivot.RotateAround(
		transform.Find("point_pivot").position, 
		transform.Find("point_pivot").rotation * Vector3(0,1,0),
		rotation_y.state);
}
...
function UpdateDrone() {
	...
	transform.Find("bottom rotor").localEulerAngles.y = bottom_rotor_rotation;
	transform.Find("top rotor").localEulerAngles.y = top_rotor_rotation;
